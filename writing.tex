...
\begin{enumerate}
  \item The Star-Transformer \textbf{divides the labor of} semantic
  compositions \textbf{between} the radical \textbf{and}
  the ring connections. \cite{star-transformer}
\end{enumerate}

\section{Trade-off}
\begin{enumerate}
  \item The need for compactness and completeness \textbf{are plainly at odds with
  each other} such that existing KG generation techniques \textbf{fail
  to satisfy both objectives properly}. \cite{practical_re} 
\end{enumerate}

\section{Our model}
\begin{enumerate}
  \item \textbf{Our model builds on a recent}
  coreference resolution \textbf{model} (Lee et al., 2017),
  \textbf{by making central use of} learned, contextualized
  span representations. \cite{srl_joint}

  \item We \textbf{use} Transformer (Vaswani et al., 2017) \textbf{as the basic building block.} \cite{levenshtein}
  
  \item We \textbf{cast
  the task as a} multi-turn question answering
  \textbf{problem}, \textbf{i.e., the extraction} of entities and relations
  \textbf{is transformed to the task of} identifying
  answer spans from the context. \cite{re_as_qa}

  \item \textbf{Encoding Layer}. The encoding layer aims to compose the input embeddings of a given instance into its corresponding instance embedding.   In this study, we choose two convolutional neural architectures, CNN (Zeng et al., 2014) and
  PCNN (Zeng et al., 2015) to encode input em
  beddings into instance embeddings. \textbf{Other neural architectures such as recurrent neural networks (Zhang and Wang, 2015) can also be used as sentence encoders. Because previous works show that both convolutional and recurrent architectures can achieve comparable state-of-the-art performance, we select convolutional architectures in this study. Note that, our model is independent of the encoder choices, and can, therefore, be easily adapted to fit other encoder architectures.} \cite{longtail_dsre}
\end{enumerate}
